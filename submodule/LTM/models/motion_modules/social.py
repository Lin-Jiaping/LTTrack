...
def SocialFeatures(x):
    N = x.shape[0]
    if N == 1:
        x_ver_repeat = x[:, -1].unsqueeze(0)
        x_hor_repeat = x[:, -1].unsqueeze(1)
    else:
...
class SocialAttention(nn.Module):
    ...
    def forward(self, in_xy, in_dxdy, enc_h, sub_batches):
        b = enc_h.size(0)
        inp = torch.cat((in_xy[-1][:, :2], in_dxdy[-1][:, :2]), -1).unsqueeze(1)
        # Shape: seq_numx1x4
        social_feats = SocialFeatures(inp)
        emb_social_feats = self.feature_embedder(social_feats)
