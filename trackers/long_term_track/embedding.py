...
from external.fast_reid.fastreid_adaptor import FastReID
...
class EmbeddingComputer:
    ...
    def compute_embedding(self, img, bbox, tag):
        ...
        results[:, 2] = results[:, 2].clip(1, w)
        results[:, 3] = results[:, 3].clip(1, h)
    ...
    def initialize_model(self):
        if "mot17" in self.dataset.lower():
            if self.test_dataset:
                path = "./weights/reid/mot17_sbs_S50.pth"
            else:
                return self._get_general_model()
        elif self.dataset == "mot20":
            if self.test_dataset:
                path = "./weights/reid/mot20_sbs_S50.pth"
            else:
                return self._get_general_model()
        elif self.dataset == "dance":
            path = "./weights/reid/dance_sbs_S50.pth"
        else:
    ...
    def _get_general_model(self):
        ...
        sd = torch.load("./weights/reid/osnet_ain_ms_d_c.pth.tar")["state_dict"]
